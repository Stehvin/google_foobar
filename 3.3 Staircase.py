# -*- coding: utf-8 -*-
"""
Created on Sun May  7 18:37:45 2017
...
udget. You can buy different amounts of the different types of bricks (for 
...
def answer(n):
    '''
    Inputs: n (int), number of blocks to be used for stairs.
    Outputs: an integer represent the different possible 
            combinations of step heights, if all blocks are
            used to create a staircase, with no repeated
            step heights.
            
    Procedure will use a generating function for the partition
    of n with distinct parts: (1 + x)(1 + x^2)...(1 + x^n)
    '''
    
    # Initialize list of generating function coefficients
    coList = [0]*(n+1)
    for i in range(4):
        coList[i] = 1
        
    # Iterate through generating function terms
    for i in range(3, n+1):
        # Copy coefficient list
        tmpCoList = coList[:]
        
        # Change each element in the coefficient list
        for j in range(i, n+1):
            coList[j] = tmpCoList[j] + tmpCoList[j-i]
